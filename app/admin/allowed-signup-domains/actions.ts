...
export async function createAllowedSignupDomain(formData: FormData): Promise<ActionResult> {
  try {
...
export async function updateAllowedSignupDomain(formData: FormData): Promise<ActionResult> {
  try {
    const res = await requireSuperadmin();
    if (!res.ok) return err("Not authorized");
...
export async function deleteAllowedSignupDomain(formData: FormData): Promise<ActionResult> {
  try {
    const res = await requireSuperadmin();
    if (!res.ok) return err("Not authorized");
