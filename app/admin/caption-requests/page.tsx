...
async function fetchFirstPageFromAnyTable(admin: ReturnType<typeof createSupabaseAdminClient>) {
  const candidates = ["caption_requests", "caption_request", "captionRequests"];
  ...
  // Retry without ordering (in case created_datetime_utc isn't present)
  for (const table of candidates) {
    const res = await admin
      .from(table)
      .select("*")
  ...
  if (!table) {
    return (
      <div className="flex flex-col gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Caption requests</h1>
              ...
              {rows.map((r, idx) => (
                ...
                >
                  {columns.map((c) => {
                    ...
                    const isLong = typeof text === "string" && text.length > 240;
                    return (
